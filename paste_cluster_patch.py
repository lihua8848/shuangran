...
def patchify(ndpa_file, ndpi_file, save_dir):
    ...
    # ax.invert_yaxis()
    # plt.show()
    # paste restored_png to slide
    slide_png = slide.read_region((0, 0), 3, slide.level_dimensions[3])
    #转成PIL格式
    slide_png = slide_png.convert('RGB')
            # 遍历每个网格单元
        ...
        for j, x_point in enumerate(x_coords):
            for k, y_point in enumerate(y_coords):
                ...
                if multipolygon_all.intersects(tile_polygon):
                    ...
                    colorfy_png = file_dir.replace('data', 'colorfy_data_vit') + f'/patch_{i}_{j}_{k}.png'
                    restored_png = Image.open(colorfy_png)

                    # plt.imshow(slide_png)
                    # plt.show()
                    restored_png = restored_png.resize((int(patch_size / slide_downsamples[3]), int(patch_size / slide_downsamples[3])))
                    slide_png.paste(restored_png, (int(x_point / slide_downsamples[3]), int(y_point / slide_downsamples[3])))
                    num_patch += 1
                    # plt.imshow(slide_png)
                    # plt.show()
    # slide.save(file_dir.replace('data', 'colorfy_slide') + ndpi_file.split('/')[-1])
    if not os.path.exists(file_dir.replace('data', 'colorfy_vit_slide')):
        os.makedirs(file_dir.replace('data', 'colorfy_vit_slide'))
    slide_png.save(file_dir.replace('data', 'colorfy_vit_slide') + '/' + ndpi_file.split('/')[-1].split('.')[0] + '.png')
    return num_patch
