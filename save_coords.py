...
def patchify(ndpa_file, ndpi_file, save_dir):
    ...
    #save file_dir + f'/patch_{i}_{j}_{k}.png'
    png_path = []
            # 遍历每个网格单元
        ...
        for j, x_point in enumerate(x_coords):
            for k, y_point in enumerate(y_coords):
                ...
                if multipolygon_all.intersects(tile_polygon):
                    ...
                    # num_patch += 1
                    # with open('/home/omnisky/workspace/shuangran/patch_coordinates.csv', 'a', newline='') as csvfile:
                    #     csvwriter = csv.writer(csvfile)
                    #     csvwriter.writerow([i, j, k, x_point, y_point])
                    png_path.append(file_dir + f'/patch_{i}_{j}_{k}.png')
    #save png_path
    with open('/home/omnisky/workspace/shuangran/patch_coordinates_png_path.csv', 'a', newline='') as csvfile:
        csvwriter = csv.writer(csvfile)
        for png in png_path:
            csvwriter.writerow([png])
    ...
    for i, dir in enumerate(abs_dir):
        ...
        for j_a, k_i in zip(ndpa_files, ndpi_files):
            ...
            try:
                ...
                print(f'{k_i} error')
                with open('/home/omnisky/workspace/shuangran/wsi_error_name.txt', 'a') as f:
                    f.writelines(f'{k_i}\n')
